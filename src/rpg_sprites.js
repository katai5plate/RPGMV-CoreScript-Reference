 ...
 * @abstract
 * 
 * @prop {Array<Sprite_Animation>} _animationSprites The animation sprites assigned to the
     * sprite object.
 * @prop {Sprite_Base} _effectTarget The target that will have the animations applied
     * to it.
 * @prop {Boolean} _hiding Property determining the sprite's visibility.
 */
 ...
 * @prop {Rectangle} _coldFrame
 * @prop {Rectangle} _hotFrame
 * @prop {Function} _clickHandler
 */
 ...
 * 
 * @prop {Game_Character} _character The Game_Character object assigned
     * to the sprite.
 * @prop {Number} _balloonDuration
 * @prop {Number} _tilesetId
 * @prop {Sprite} _upperBody
 * @prop {Sprite} _lowerBody
 * @prop {Number} _bushDepth
 * @prop {Sprite_Balloon} _balloonSprite The current balloon sprite
 ...
 * 
 * @prop {Game_Battler} _battler
 * @prop {Array<Sprite_Damage>} _damages
 * @prop {Number} _homeX
 * @prop {Number} _homeY
 * @prop {Number} _offsetX
 * @prop {Number} _offsetY
 * @prop {Number} _targetOffsetX
 * @prop {Number} _targetOffsetY
 * @prop {Number} _movementDuration
 * @prop {Number} _selectionEffectCount
 */
 ...
 * @param {Game_Battler} [battler]
 * 
 * @prop {Object} MOTIONS [static]
 * @prop {MV.Motion} MOTIONS.walk
 * @prop {MV.Motion} MOTIONS.wait
 * @prop {MV.Motion} MOTIONS.chant
 * @prop {MV.Motion} MOTIONS.guard
 * @prop {MV.Motion} MOTIONS.damage
 * @prop {MV.Motion} MOTIONS.evade
 * @prop {MV.Motion} MOTIONS.thrust
 * @prop {MV.Motion} MOTIONS.swing
 * @prop {MV.Motion} MOTIONS.missile
 * @prop {MV.Motion} MOTIONS.skill
 * @prop {MV.Motion} MOTIONS.spell
 * @prop {MV.Motion} MOTIONS.item
 * @prop {MV.Motion} MOTIONS.escape
 * @prop {MV.Motion} MOTIONS.victory
 * @prop {MV.Motion} MOTIONS.dying
 * @prop {MV.Motion} MOTIONS.abnormal
 * @prop {MV.Motion} MOTIONS.sleep
 * @prop {MV.Motion} MOTIONS.dead
 * 
 * @prop {String} _battlerName
 * @prop {MV.Motion} _motion
 * @prop {Number} _motionCount
 * @prop {Number} _pattern
 * @prop {Sprite_Base} _mainSprite
 * @prop {Sprite} _shadowSprite
 * @prop {Sprite_Weapon} _weaponSprite
 * @prop {Sprite_StateOverlay} _stateSprite
 ...
 * @class Sprite_Enemy
 * @extends Sprite_Battler
 * @prop {Game_Enemy} battler
 * 
 * @prop {Game_Enemy} _enemy
 * @prop {Boolean} _appeared
 * @prop {String} _battlerName
 * @prop {Number} _battlerHue
 * @prop {String} _effectType
 * @prop {Number} _effectDuration
 * @prop {Number} _shake
 * @prop {Sprite_StateIcon} _stateIconSprite
 */
 ...
 * @extends Sprite
 * 
 * @prop {Object} _checker1 [static]{key: RPG.Animation}
 * @prop {Object} _checker2 [static]{key: RPG.Animation}
 * 
 * @prop {Number} z
 * 
 * @prop {Sprite_Base} _target
 * @prop {RPG.Animation} _animation
 * @prop {Boolean} _mirror
 * @prop {Number} _delay
 * @prop {Number} _rate
 * @prop {Number} _duration
 * @prop {Array<Number>} _flashColor
 * @prop {Number} _flashDuration
 * @prop {Number} _screenFlashDuration
 * @prop {Number} _hidingDuration
 * @prop {Bitmap} _bitmap1
 * @prop {Bitmap} _bitmap2
 * @prop {Array<Sprite>} _cellSprites
 * @prop {ScreenSprite} _screenFlashSprite
 * @prop {Boolean} _duplicated
 * @prop {Boolean} _reduceArtifacts
 */
...
/**
 * @param  {Array<Array<Number>>} frame
 */
 ...
 * @extends Sprite
 * 
 * @prop {Number} _iconWidth [static]
 * @prop {Number} _iconHeight [static]
 * 
 * @prop {Game_Battler} _battler
 * @prop {Number} _iconIndex
 * @prop {Number} _animationCount
 * @prop {Number} _animationIndex
 */
 ...
 * 
 * @prop {Number} _pictureId
 * @prop {String} _pictureName
 * @prop {Boolean} _isPicture
 */
...
/**
 * {@link Spriteset_Map} と {@link Spriteset_Battle} のスーパークラス。
 * @class Spriteset_Base
 * @extends Sprite
 * @abstract
 * 
 * @prop {Array<Number>} _tone
 * @prop {Sprite} _baseSprite
 * @prop {ScreenSprite} _blackScreen
 * @prop {ToneFilter} _toneFilter
 * @prop {ToneSprite} _toneSprite
 * @prop {Sprite} _pictureContainer
 * @prop {Sprite_Timer} _timerSprite
 * @prop {ScreenSprite} _flashSprite
 * @prop {ScreenSprite} _fadeSprite
 */
...
/**
 * マップスプライト表示用オブジェクト。{@link Scene_Map}オブジェクトで管理される。
 * @class Spriteset_Map
 * @extends Spriteset_Base
 * 
 * @prop {TilingSprite} _parallax [遠景]スプライト
 * @prop {Tilemap | ShaderTilemap} _tilemap マップ画像本体
 * @prop {RPG.Tileset} _tileset [タイルセット]
 * @prop {Array<Sprite_Character>} _characterSprites [アクター][イベント]のスプライト
 * @prop {Sprite} _shadowSprite 影スプライト
 * @prop {Sprite_Destination} _destinationSprite タッチ位置スプライト
 * @prop {Weather} _weather 天候
 * @prop {String} _parallaxName [遠景]画像ファイル名
 */
...
/**
 * タイルマップ({@link Tilemap} か {@link ShaderTilemap})を生成。
 * @memberof Spriteset_Map
 */
...
/**
 * キャラクタスプライト({@link Sprite_Character})の生成。
 * @memberof Spriteset_Map
 */
...
/**
 * タッチ位置表示スプライト({@link Sprite_Destination})の生成。
 * @memberof Spriteset_Map
 */
 ...
 * @extends Spriteset_Base
 * 
 * @prop {Boolean} _battlebackLocated
 * @prop {Sprite} _backgroundSprite
 * @prop {Sprite} _battleField
 * @prop {TilingSprite} _back1Sprite
 * @prop {TilingSprite} _back2Sprite
 * @prop {Array<Sprite_Enemy>} _enemySprites
 * @prop {Array<Sprite_Actor>} _actorSprites
 */
...
/**
 * @param  {Sprite_Enemy} a
 * @param  {Sprite_Enemy} b
 * @returns {Number}
 */
